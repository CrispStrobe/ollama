...
// Model implements XLM-RoBERTa encoder for embeddings.
// Like BERT but without type embeddings and with a position offset.
type Model struct {
	model.Base
	tokenizer.Tokenizer
...
func (m *Model) Forward(ctx ml.Context, batch input.Batch) (ml.Tensor, error) {
	...
	for i, p := range batch.Positions {
		positions[i] = p + int32(m.positionOffset)
	}
	hiddenStates = hiddenStates.Add(ctx, m.PositionEmbedding.Forward(ctx, ctx.Input().FromInts(positions, len(positions))))
...
func New(c fs.Config) (model.Model, error) {
	...
	return &Model{
		...
		Options: Options{
			...
			positionOffset: int(c.Uint("position_offset", 2)),
...
func init() {
	model.Register("xlmr", New)
	model.Register("xlmr_embed", New)
}
